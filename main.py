import os import csv from datetime import datetime, timedelta from threading import Thread from flask import Flask from telegram import InlineKeyboardButton, InlineKeyboardMarkup, Update, Bot from telegram.ext import ApplicationBuilder, CommandHandler, CallbackQueryHandler, ContextTypes from apscheduler.schedulers.background import BackgroundScheduler import pandas as pd

TOKEN = os.getenv("BOT_TOKEN") ADMIN_CHAT_ID = int(os.getenv("ADMIN_CHAT_ID", "123456789"))  # Replace with your actual Telegram user ID PDF_FOLDER = "pdfs" LOG_FOLDER = "logs"
...
subjects = [ "biology", "mathematics", "physics", "communication_skill", "electrical_engineering", "mechanical_engineering", "environmental_science" ]

Log download to CSV and notify admin

def log_download(user, subject, exam_type, year): os.makedirs(LOG_FOLDER, exist_ok=True) log_file = os.path.join(LOG_FOLDER, "downloads.csv")
...
def clean_old_logs(): log_file = os.path.join(LOG_FOLDER, "downloads.csv") if not os.path.isfile(log_file): return
...
def send_report_to_admin(): log_file = os.path.join(LOG_FOLDER, "downloads.csv") if not os.path.isfile(log_file): return
...
def start_scheduler(): scheduler = BackgroundScheduler() scheduler.add_job(send_report_to_admin, 'cron', day_of_week='mon', hour=9, minute=0)  # Weekly on Monday 9 AM scheduler.start()

Telegram Bot Commands

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): keyboard = [[InlineKeyboardButton(subj.replace("_", " ").title(), callback_data=subj)] for subj in subjects] reply_markup = InlineKeyboardMarkup(keyboard)
...
async def subject_handler(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query await query.answer()
...
async def ask_year(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query await query.answer()
...
async def send_exam_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query await query.answer()
...
async def unit_note_handler(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query await query.answer() unit = query.data subject = context.user_data.get("subject")
...
def main(): keep_alive() clean_old_logs() start_scheduler()
